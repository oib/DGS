...
# A simple systemd-like service manager for the production server

SERVICE_NAME="dgs-platform"
SERVICE_DIR="/var/www/dgs"
PID_FILE="/tmp/dgs-platform.pid"
LOG_FILE="$SERVICE_DIR/logs/dgs-platform.log"
...
case "$1" in
    start)
        ...
        cd "$SERVICE_DIR"
        PORT=3100 HOSTNAME=0.0.0.0 nohup npm start > "$LOG_FILE" 2>&1 &
        echo $! > "$PID_FILE"
